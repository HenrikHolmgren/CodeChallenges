...
{
    public class Day22 : AdventOfCode2018SolverBase
    {
        ...
        public Day22(IIOProvider provider) : base(provider) { }
        ...
        public override string ExecutePart2()
        {
            ...
            while (Fringe.Any())
            {
                ...
                {
                    if (candidate.x + neighbour.x < 0)
                        continue;
                    if (candidate.y + neighbour.y < 0)
                        continue;
                    if (candidate.x + neighbour.x > _target.x * 50)
                        continue;
                    if (candidate.y + neighbour.y > _target.y * 100)
                        continue;
